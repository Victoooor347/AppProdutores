...
import { formatDate, formatDateInput, dateInputToIso } from '../utils/format';
...
const CULTURA_OPTIONS = [
  { label: 'Culturas', value: 'todas' as const },
...
export default function Relatorios() {
  ...
  const loadDados = useCallback(async () => {
    ...
    try {
      ...
      setCatalogoIEs(
        Array.from(new Set(catalogoData.data.map((carga) => carga.inscricaoEstadual)))
      );
  ...
  }, [token, ano, cultura, inscricaoEstadual, dataInput]);
        ...
        <View style={style.resumoRowRel}>
          <View style={style.resumoCardRel}>
            ...
            <Text style={style.resumoValueRel}>
              ...
              <Text style={style.resumoUnidadeRel}>{resumoDoCultura('arroz')?.unidade ?? 'sc'}</Text>
            </Text>
          </View>
          <View style={style.resumoCardRel}>
            ...
            <Text style={style.resumoValueRel}>
              ...
              <Text style={style.resumoUnidadeRel}>{resumoDoCultura('soja')?.unidade ?? 'sc'}</Text>
            </Text>
          </View>
        </View>

        <View style={style.filterBarRel}>
          <SelectField label="IE" value={inscricaoEstadual} options={ieOptions} onChange={setInscricaoEstadual} />
          <SelectField label="Cultura" value={cultura} options={CULTURA_OPTIONS} onChange={setCultura} />
          <View style={style.dateFieldRel}>
            <TextInput
              style={style.dateInputRel}
              placeholder="Data"
              placeholderTextColor={themes.colors.cinzaMedio}
              value={dataInput}
              onChangeText={(text) => setDataInput(formatDateInput(text))}
